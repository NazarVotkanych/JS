  ...
  debugger;
   for (const user of usersList) {
        ...
        for (const key in user ) {
            ...
            divText.innerText = user[key];
            if (typeof user[key] !== 'object') {
                userBlock.appendChild(divText);
            } else {
                for (const key1 in user[key]) {
                    let div1Text = document.createElement('div')
                    div1Text.innerText = user[key][key1]
                    if (typeof user[key][key1] !== 'object') {
                        userBlock.appendChild(div1Text)
                    } else {
                        for (const key2 in user[key][key1]) {
                            let div2Text = document.createElement('div')
                            div2Text.innerText = user[key][key1][key2]
                            if (user[key][key1][key2] !== 'object') {
                                userBlock.appendChild(div2Text)
                            }
                        }
                    }
                }
